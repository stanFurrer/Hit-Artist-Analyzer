...
function display_song(file, song) {
	d3.csv(file,function(data) {
          ...
          .duration(1000)
		      ...
		      .attr("d", d3.line()
		        .x(function(d) { return x(d.group) })
		        .y(function(d) { return y(d['The Beatles']) })
          );
        svg
          .append("path")
          .datum(data)
          ...
          .attr("d", d3.line()
            .x(function(d) { return x(d.group) })
            .y(function(d) { return y(d['Frank Sinatra']) })
          );
		})
}
...
function update(selectedVar) {
    ...
    if(selectedVar == "Yesterday"){
      display_song("/data/yesterday_sin_eminem.csv", selectedVar)
    } else {
      display_song("/data/something_sin_eminem.csv", selectedVar)
    }
}

display_conf_interval("/data/Frank Sinatra_mean_std.csv", colors[0])
display_conf_interval("/data/The Beatles_mean_std.csv", colors[2])
update(songs[0])
